...
class SimpleUnitTest(unittest.TestCase):
    def setUp(self):
        section_base_proxy().set_database_name('EU')
        section_base_proxy().set_unit_system(length='m', mass_unit='kg', force_unit='N')

    def test_HE100B_EU(self):
        # HEB 100 B has mass=20.4kg/m Ax=26.036cm2 Iy=449.5cm4 Iz=167.3cm4

        section_base_proxy().set_unit_system(length='m', mass_unit='kg', force_unit='N')
        param = section_base_proxy().get_section_parameters('HE 100 B')
        ...
        section_base_proxy().set_unit_system(length='cm', mass_unit='kg', force_unit='N')
        param = section_base_proxy().get_section_parameters('HE 100 B')
        ...
        section_base_proxy().set_unit_system(length='mm', mass_unit='kg', force_unit='N')
        param = section_base_proxy().get_section_parameters('HE 100 B')
        ...
        section_base_proxy().set_unit_system(length='ft', mass_unit='kg', force_unit='N')
        param = section_base_proxy().get_section_parameters('HE 100 B')
        ...
        section_base_proxy().set_unit_system(length='inch', mass_unit='kg', force_unit='N')
        param = section_base_proxy().get_section_parameters('HE 100 B')
        ...
        section_base_proxy().set_unit_system(length='ft', mass_unit='lb', force_unit='N')
        param = section_base_proxy().get_section_parameters('HE 100 B')
        self.assertAlmostEqual(param['Ax'] / (26e-4 / (0.3048 ** 2)), 1, places=2)
        ...
        section_base_proxy().set_unit_system(length='ft', mass_unit='lb', force_unit='lbf')
        param = section_base_proxy().get_section_parameters('HE 100 B')
        self.assertAlmostEqual(param['Ax'] / (26e-4 / (0.3048 ** 2)), 1, places=2)
        self.assertAlmostEqual(param['mass'] / (20.4 * 0.3048 / 0.454), 1, places=2)
        self.assertAlmostEqual(param['Iy'] / (449.5e-8 / (0.3048) ** 4), 1, places=2)
    ...
    def test_W12X65_US(self):
        ...
        section_base_proxy().set_unit_system(length='inch', mass_unit='lb', force_unit='lbf')
        param = section_base_proxy().get_section_parameters('W12X65')
